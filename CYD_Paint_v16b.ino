// CYD Paint v12 — ESP32-2432S028 (ILI9341 240x320, XPT2046, SD)
...
void toolApply(int cx, int cy) {
    ...
    int x1, y1, x2, y2;
    stampBrushAt(cx, cy, drawColor, x1, y1, x2, y2);

    // Push the single dirty rectangle
    int pw = x2 - x1 + 1, ph = y2 - y1 + 1;
    tft.startWrite();
...
void toolApplyLine(int x0, int y0, int x1, int y1) {
    ...
    while (true) {
        // Clamp to canvas before stamping
...
void handleMainTouch(int tx, int ty) {
    ...
    if (rectHit(tx, ty, 0, 20, 20, CANVAS_H)) {
    ...
    if (rectHit(tx, ty, 220, 20, 20, CANVAS_H)) {
        int idx = ty / (CANVAS_H / 12);
        idx = constrain(idx, 0, 11);
        ...
        if (isTwoTapTool(activeTool)) {
            ...
            if (!shapeHasOrigin) {
                ...
                if (activeTool == TOOL_LINE) {
                    drawShapeLine(shapeOriginX, shapeOriginY, cx, cy, activeColor);
                } else if (activeTool == TOOL_RECT) {
                    drawShapeRect(shapeOriginX, shapeOriginY, cx, cy, activeColor);
                } else if (activeTool == TOOL_ELLIPSE) {
                    int erx = abs(cx - shapeOriginX) / 2;
                    int ery = abs(cy - shapeOriginY) / 2;
                    int ecx = (shapeOriginX + cx) / 2;
                    int ecy = (shapeOriginY + cy) / 2;
