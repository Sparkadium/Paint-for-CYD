// CYD Paint v21 — ESP32-2432S028 (ILI9341 240x320, XPT2046, SD)
...
void drawStatusBar() {
    ...
    if (statusMsg[0]) {
        ...
        tft.setCursor(CANVAS_X + 2, STATUS_Y + 6);
        tft.printf("I:%d A:%d %dKB", imgCount, animCount, (int)(ESP.getFreeHeap()/1024));
    }
}
...
void animDrawFilmstrip() {
    ...
    for (int v = 0; v < maxV; v++) {
        ...
        uint16_t numCol = cur ? C_YELLOW : (isSrc ? (animOp == AOP_MOVE ? C_YELLOW : C_MAGENTA) : C_GRAY);
        tft.setTextColor(numCol, 0x1082);
        tft.setTextSize(1); tft.setCursor(sx + 2, ANIM_STRIP_Y + 2);
        tft.print(fi + 1);
        // Draw colored border for source frame
        if (isSrc && !cur) {
            uint16_t bdrCol = (animOp == AOP_MOVE) ? C_YELLOW : C_MAGENTA;
            tft.drawRect(sx, ANIM_STRIP_Y + 2, ANIM_THUMB_W, ANIM_THUMB_H, bdrCol);
            tft.drawRect(sx+1, ANIM_STRIP_Y + 3, ANIM_THUMB_W-2, ANIM_THUMB_H-2, bdrCol);
        }
    }
}
...
void drawAnimScreen() {
    ...
    // Row 1 (y=266): nav + frame add/remove
    int r1 = 266;
    ...
    drawButtonRect(136, r2, 40, 18, onionSkin ? 0x03EF : 0x4208, onionSkin ? C_BLACK : C_CYAN, "ONION");
    drawButtonRect(176, r2, 32, 18, 0x4208, C_YELLOW, "DRAW");
    drawButtonRect(208, r2, 32, 18, 0x4208, C_ORANGE, "SAVE");

    // Row 3 (y=306): quick info / cancel op
    int r3 = 306;
    if (animOp != AOP_NONE) {
...
void handleAnimTouch(int tx, int ty) {
    ...
    if (rectHit(tx, ty, 0, r2, 240, 18)) {
        ...
        else if (rectHit(tx,ty,136,r2,40,18)) { onionSkin = !onionSkin; drawAnimScreen(); }
        else if (rectHit(tx,ty,176,r2,32,18)) { animOp=AOP_NONE; drawMainScreen(); }
        else if (rectHit(tx,ty,208,r2,32,18)) { animOp=AOP_NONE; animSaveAll(); drawAnimScreen(); }
        return;
    }
}
