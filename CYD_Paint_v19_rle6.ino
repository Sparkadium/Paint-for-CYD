// CYD Paint v19 — ESP32-2432S028 (ILI9341 240x320, XPT2046, SD)
...
// Gallery state
int  galleryPage = 0;
char galleryFiles[32][16];      // image mode: filename e.g. "PAINT000.BMP"
...
void drawBottomBar() {
    ...
    drawButtonRect(80, ry, 110, 30, 0x4208, C_YELLOW, "COLOUR PICK");
    { char lbl[10]; snprintf(lbl, 10, "ANIM%d", animFrameCount);
      drawButtonRect(190, ry, 50, 30, onionSkin ? 0x03EF : 0x4208,
...
void handleMainTouch(int tx, int ty) {
    ...
    if (ry < 60) {
        ...
        if (rectHit(tx, localY, 80, 0, 110, 30)) {  // COLOUR PICK (narrowed)
            cpSlot = 0;
            uint8_t r, g, b; rgb565to888(activeColor, r, g, b);
            cpR = r; cpG = g; cpB = b;
            drawColorPickScreen(); return;
        }
        if (rectHit(tx, localY, 190, 0, 50, 30)) {  // ANIM button
            animCommitFrame();
...
void scanAnims() {
    ...
    char seen[32][7];  // up to 32 unique prefixes, "ANMnnn" + null
    int seenCount = 0;
    File root = SD.open("/");
    while (true) {
        File f = root.openNextFile();
    ...
    for (int i = 0; i < seenCount && i < 32; i++) {
...
void drawGalleryScreen() {
    ...
    tft.setTextSize(1);
    tft.setCursor(4, 4);
    const char *modeLabel = galleryShowAnims ? "ANIMATIONS" : "IMAGES";
    tft.printf("%s  Page %d/%d", modeLabel, galleryPage + 1, max(1, (galleryCount + 3) / 4));

    // 2x2 grid of entries
    int startIdx = galleryPage * 4;
    for (int i = 0; i < 4 && startIdx + i < galleryCount; i++) {
        int col = i % 2, row = i / 2;
        int ex = col * 120, ey = 18 + row * 140;
        tft.drawRect(ex, ey, 110, 110, galleryShowAnims ? C_CYAN : C_WHITE);
        tft.setTextColor(C_GRAY, 0x1082);
        tft.setCursor(ex + 2, ey + 112);
        tft.print(galleryFiles[startIdx + i]);
        drawButtonRect(ex,      ey + 124, 55, 20, 0x4208, C_GREEN, "LOAD");
        drawButtonRect(ex + 55, ey + 124, 55, 20, 0x4208, C_RED,   "DEL");
    }

    // Nav bar: <PREV | IMGS/ANMS toggle | NEXT> | BACK
    drawButtonRect(0,   300, 50, 20, 0x4208, C_WHITE,  "<PREV");
    drawButtonRect(50,  300, 70, 20, galleryShowAnims ? C_CYAN : 0x4208,
                                    galleryShowAnims ? C_BLACK : C_CYAN,
                                    galleryShowAnims ? "ANIMS" : "IMAGES");
    drawButtonRect(120, 300, 60, 20, 0x4208, C_WHITE,  "NEXT>");
    drawButtonRect(180, 300, 60, 20, 0x4208, C_ORANGE, "BACK");
}

void handleGalleryTouch(int tx, int ty) {
    // Nav bar
    if (rectHit(tx, ty, 180, 300, 60, 20)) { drawMainScreen(); return; }
    if (rectHit(tx, ty, 0,   300, 50, 20)) {
        if (galleryPage > 0) { galleryPage--; drawGalleryScreen(); }
        return;
    }
    if (rectHit(tx, ty, 120, 300, 60, 20)) {
        if ((galleryPage + 1) * 4 < galleryCount) { galleryPage++; drawGalleryScreen(); }
        return;
    }
    // Toggle images / animations
    if (rectHit(tx, ty, 50, 300, 70, 20)) {
